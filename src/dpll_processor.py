...
from src.type_defs import DPLLTable, DPLLClause


def dpll(
    clause_table: DPLLTable,
    variables: int,
    starting: int
) -> bool:
    """
...
def _self_subsuming_resolution_recursive(clause_table: DPLLTable) -> DPLLTable:
    ...
    for i in range(len(clause_table)):
        for j in range(i, len(clause_table)):
            resolvable, value = _can_be_resolved(clause_table[i], clause_table[j])
            if resolvable:
                resolved_clause = _resolve_clause_subsets(clause_table[i], clause_table[j], value)
